...
#include "ui_graphicTracePlot.h"
#include "ui_startstopemuldata.h"

GraphicTracePlot::GraphicTracePlot(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::GraphicTracePlot),
    _timeInterval(TIMER_REFRESH)
{
    ...
    _emulData->show();
    _emulData->setTimeInterval(TIMER_REFRESH);
    connect(_emulData,SIGNAL(refreshPlot()),this, SLOT(realtimeDataSlot()));
#endif
    //set margin
...
GraphicTracePlot::~GraphicTracePlot()
{
    delete ui;
}

void GraphicTracePlot::setMaxValue(int value)
{
    _maxValue = value;
}

int GraphicTracePlot::getMinValue() const
{
    return _minValue;
}

int GraphicTracePlot::getMaxValue() const
{
    return _maxValue;
}

void GraphicTracePlot::setMinValue(int value)
{
    _minValue = value;
}


void GraphicTracePlot::setupPlot()
{
    //setup the style
...
void GraphicTracePlot::setupStyle(QCustomPlot *customPlot)
{
...
void GraphicTracePlot::setupTrace(QCustomPlot *customPlot)
{
    //trace
    customPlot->addGraph();
...
#if IN_DOOR_DATA
void GraphicTracePlot::updatePlot()
{
...
void GraphicTracePlot::updatePlot()
{
    ...
    //                QDateTime(QDate::currentDate()).toMSecsSinceEpoch())/TIMER_REFRESH;
    //Add the CPT in x data buffer
    _XData.append(_emulData->frameData()->getCPT());
    ...
    //ui->tracePlot->yAxis->setRange(yPlotMin , yPlotMax);
    // make key axis range scroll with the data:
    ui->tracePlot->xAxis->setRange(_emulData->frameData()->getCPT(), NB_X_VALUES_DISPLAY_LIVE, Qt::AlignRight);
...
void GraphicTracePlot::realtimeDataSlot()
{
#if IN_DOOR_DATA
...
#else
    static qint64 key = 0;
    static int refreshPlot = DISPLAY_REFRESH;
#endif
    _XData.append(key);
    ...
    double key = 0;
#else
   quint64 key = _emulData->frameData()->getCPT();
   static int refreshPlot = DISPLAY_REFRESH;
#endif
    _XData.append(key);
 //   static double value = 0;
